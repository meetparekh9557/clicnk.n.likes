...
- [x] duplicate-content-penalty — Is There a Duplicate Content Penalty? What Google Actually Does — links: hub (SEO services), siblings `how-to-add-schema-markup`, `blog-that-ranks` — ends in `/tools/content-score/`. Written to support the founder's LinkedIn post on the myth, so he can link to it as the long-form reference. Demand check Aug 2026: strong signal, Google's own 2008 'Demystifying the duplicate content penalty' post ranks alongside Backlinko and several small sites - contested but not locked. Dated 2026-08-18 at the founder's explicit request (backdated to fill the empty Tue 18 Aug slot). Carries the nuance the LinkedIn post omits: scraped content republished without added value IS covered by Google's spam policies, so the 'nothing gets flagged' framing has one real exception.
