...
// Multiple services take a tiered discount (2:15% 3:22% 4+:30%); duration decides free
...
import { getCurrency, onCurrency, usableCurrency, priceIn, money as fmtMoney, MARKET_RATE } from '../../lib/currency.js';
...
export default function QuoteCalculator({ preselect, thankYouHref }) {
  ...
  // Formats an amount ALREADY in the display currency - the pricing block
  // below computes in `shown`, so nothing here converts.
  const money = (n) => fmtMoney(n, shown);
  // For the handful of places that still hold an INR figure (tier buttons and
  // the per-unit add-on rates, which are authored in INR): convert first.
  // Kept as a separate name on purpose - money() no longer converts, so
  // passing it an INR amount silently renders "$16,000" for a $500 tier.
  ...
  const lines = [];
  // Every figure is computed TWICE: once in the display currency (what the
  // visitor sees and what must add up on screen) and once in INR (the base
  // the engagement is recorded and invoiced in, used by the emailed quote).
  // Totalling in INR and converting at the end is what previously put four
  // $500 line items above a $1,651 subtotal, because a curated tier price is
  // deliberately not the INR base times a rate.
  const rate = MARKET_RATE[shown] ?? 1;
  let monthlySub = 0, onetimeSub = 0, hasCustom = false;
  let monthlySubInr = 0, onetimeSubInr = 0;
  sel.forEach((k) => {
    ...
    let amt = priceIn(tier.price, shown);
    let amtInr = tier.price;
    UNITS[k].forEach((u) => {
      const extra = Math.max(0, getQty(k, u) - incOf(u, getTier(k)));
      const unitInr = extra * unitRate(u, getWords(k, u));
      amtInr += unitInr;
      amt += unitInr * rate;
    });
    if (SERVICES[k].kind === 'onetime') { onetimeSub += amt; onetimeSubInr += amtInr; }
    else { monthlySub += amt; monthlySubInr += amtInr; }
    lines.push({ label: SERVICES[k].label, amt, amtInr, tier: tier.name, unit: SERVICES[k].kind === 'onetime' ? 'one-time' : '/month' });
  });
  // Multi-service discount steepens with the number of services. A flat
  // 15% was fine when a top-tier service was ₹1.24L, but once each market's
  // tier is priced to that market, four services at Pro compound into an
  // enterprise number no new agency can credibly quote. Steepening keeps a
  // full-stack engagement in a range a real buyer will actually consider,
  // and it reflects the genuine efficiency of running the channels together
  // rather than being a discount invented to rescue the arithmetic.
  const multi = sel.length >= 4 ? 0.30 : sel.length === 3 ? 0.22 : sel.length === 2 ? 0.15 : 0;
  const monthlyNet = Math.round(monthlySub * (1 - multi));
  const onetimeNet = Math.round(onetimeSub * (1 - multi));
  const monthlyNetInr = Math.round(monthlySubInr * (1 - multi));
  const onetimeNetInr = Math.round(onetimeSubInr * (1 - multi));
  const monthlyForDurationInr = monthlyNetInr * dur.charged;
  const grandInr = monthlyForDurationInr + onetimeNetInr;
  ...
  // What the client keeps: the free months, plus the multi-service discount across the paid
  ...
  if (multi) notes.push(`A ${Math.round(multi * 100)}% multi-service discount is applied for running ${sel.length} services together.`);
  ...
  function submit(evt) {
    ...
    if (multi) factors.push(fact('Multi-service discount', `${Math.round(multi * 100)}% off`, 'self', 'applied'));
    ...
    sendFromClicknlikes({
      ...
      bodyText: `New plan:\n\nCame from page: ${pagePath}\n\nName: ${name}\nBusiness: ${business}\nEmail: ${email}\nPhone: ${phone || '-'}\nCity: ${city || '-'}\nCountry: ${country || '-'}\nViewed in: ${cur}\nDuration: ${dur.m} months (pay ${dur.charged})\nDiscount: ${Math.round(multi * 100)}% (INR base)\n\nLines (INR):\n${lines.map((l) => `  ${l.label}${l.tier ? ` (${l.tier})` : ''}: ${l.custom ? 'Custom' : inr(l.amtInr) + ' ' + l.unit}`).join('\n')}\n\nMonthly net: ${inr(monthlyNetInr)} | Over ${dur.m}mo: ${inr(monthlyForDurationInr)} | One-time: ${inr(onetimeNetInr)} | Grand: ${inr(grandInr)}`,
      fields: {
        form: 'plan-builder',
        page: pagePath,
        name, business, email,
        ...
        monthly_inr: monthlyNetInr,
        onetime_inr: onetimeNetInr,
        total_inr: grandInr,
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {TIERS.map((t) => (
                    <button key={t.id} type="button"
                      ...
                      {t.price == null ? 'Custom →' : `${t.name} ${priceOf(t.price)}`}
                    </button>
                  ))}
                </div>
                    ...
                    return (
                      <div key={u.key} className="mt-3">
                        <div className="flex items-center justify-between text-[12.5px]">
                          ...
                          <span className="font-display font-bold text-teal-dark tabular-nums">{q}{extra > 0 ? ` (+${priceOf(extra * unitRate(u, w))})` : ''}</span>
                        </div>
                        ...
                        <p className="text-[11px] text-navy/45">{inc} included · {priceOf(u.price)} each extra{u.type === 'wordblog' ? ` up to ${u.includedWords} words` : ''}</p>
      ...
      <div className="flex flex-col">
        <div className="rounded-xl border border-teal/40 bg-teal/[0.06] p-5">
          ...
          <ul className="mt-4 space-y-1.5 border-t border-navy/10 pt-3 text-[12.5px] text-navy/65">
            ...
            {sel.length > 0 && monthlySub > 0 && (
              <>
                ...
                {multi > 0 && <li className="flex justify-between gap-2 font-semibold text-teal-dark"><span>Multi-service −{Math.round(multi * 100)}%</span><span className="tabular-nums whitespace-nowrap">−{money(monthlySub * multi)}/mo</span></li>}
          ...
          {sel.length > 0 && savings > 0 && (
            <div className="mt-3 rounded-lg border border-teal/40 bg-teal/10 px-3 py-2 text-[12px] font-semibold text-teal-dark">You save {money(savings)}{freeMonths ? ` · ${freeMonths} month${freeMonths > 1 ? 's' : ''} free` : ''}{multi ? ` + ${Math.round(multi * 100)}% off` : ''}</div>
          )}
          <ul className="mt-3 space-y-1.5 text-[11px] leading-relaxed text-navy/50">
            {notes.map((n, i) => <li key={i}>· {n}</li>)}
          </ul>
          {shown !== 'INR' && <p className="mt-2 text-[11px] leading-relaxed text-navy/45">Pricing is set per market, not converted at an exchange rate. Your quote is confirmed in {shown} in the written proposal.</p>}
        </div>
