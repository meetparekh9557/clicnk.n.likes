// Every country, with ISO-3166 alpha-2 and E.164 dialling code. Used by
// the shared CountrySelect + PhoneInput islands on every lead form.
//
// The full list is deliberate: a visitor from a market we do not actively
// target must still be able to identify themselves correctly rather than
// pick the nearest wrong option. Which markets we *pursue* is a marketing
// decision, and it does not belong in a form field.
//
// `dial` is the primary country calling code. A handful of territories
// share a code with their parent country (e.g. +1 across NANP members);
// that is correct and expected - the ISO code is what disambiguates them.
